<template>
  <n-space vertical class="searchpage">
    <n-space
      item-style="display:flex;margin:10px;"
      align="center"
      justify="center"
    ...
    <n-collapse arrow-placement="right" :default-expanded-names="[0]">
      <n-collapse-item
        v-for="(event, eventid, eidx) in result"
        :key="eidx"
        :name="eidx"
      >
        <template #header>
        ...
        <n-list>
          <n-list-item v-for="(story, sidx) in event" :key="sidx">
            <n-thing>
              <template #header>
                <n-text strong
                  >{{ story.storyCode }} {{ story.storyName }} </n-text
                >&nbsp;
              ...
              <template #header-extra>
                <n-button
                  secondary
                  type="info"
                  @click="loadStory(story.storyPath)"
                >
                  <n-icon size="24">
                    <OpenIcon />
                  </n-icon>
              ...
              <template #description>
                <n-blockquote
                  class="textmatch"
                  v-for="(match, midx) in story.text_matches"
                  :key="midx"
                  v-html="parseFrag(match['fragment'], match['matches'])"
                ></n-blockquote>
              </template>
            </n-thing>
          </n-list-item>
    ...
    <n-text v-if="!isSearched">
      <n-h3 prefix="bar"> 关于搜索 / About Search: </n-h3>
      ASTR提供基于Github API的搜索功能。由于Github API的限制，<n-text
        type="warning"
        >每个文件只能展示两条结果</n-text
      ><br /><br />
      ASTR provides search feature based on Github API. Due to the limitation of
      Github API,
      <n-text type="warning">each file can only show two results</n-text
      ><br /><br />
      <n-text depth="3" delete
        >归根结底还是没钱搭服务器，只能靠嫖Github过日子。</n-text
      >
    </n-text>
  </n-space>
</template>
...
export default {
  ...
  methods: {
    searchRequest() {
      ...
      var requestURL =
        "https://astr-search.azurewebsites.net/api/search?q=" +
        this.searchvalue +
        "&server=" +
        this.server +
        "/gamedata/story";
          ...
          for (storytype in this.eventList) {
            ...
            for (eventid in ctype) {
              ...
              for (storyidx in cevent["infoUnlockDatas"]) {
                var idx = resultlist.indexOf(
                  cevent["infoUnlockDatas"][storyidx]["storyTxt"]
                );
