        ...
        <div v-for="(edata, eidx) in getEventList" :key="edata.id">
          <n-list-item>
            <n-flex
              ...
              class="eventtitle"
              @click="
                $router.push('/' + $route.params.server + '/event/' + edata.id)
              "
            >
              <n-image
                v-if="eventype != 'or'"
              ...
              <n-image
                v-else
                :src="
                  'https://raw.githubusercontent.com/Aceship/Arknight-Images/main/avatars/char_' +
                  edata.cid +
                  '_' +
                  edata.cin +
                  '.png'
              ...
              <n-flex vertical justify="space-around">
                <n-flex align="baseline">
                  <n-text
                    ...
                    >{{ edata.name }}</n-text
                  >
                  <n-text depth="3" v-if="eventype == 'or'"
                    >#{{ edata.set }}</n-text
                  >
                </n-flex>
            ...
            <template #suffix>
              <OpenInNew
                :link="'/' + $route.params.server + '/event/' + edata.id"
              />
            </template>
          </n-list-item>
        </div>
